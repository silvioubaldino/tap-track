...
export function TimeTracker() {
  ...
  const handleExportData = useCallback((exportAll: boolean = false) => {
    const headers = [t('timeTracker.date'), t('timeTracker.startTime'), t('timeTracker.endTime'), t('timeTracker.duration')];
    const rows: string[][] = [];
    ...
    if (exportAll) {
      Object.entries(allIntervals).forEach(([dateKey, dayIntervals]) => {
        ...
        dayIntervals.forEach(interval => {
          rows.push([
            new Date(interval.start).toLocaleDateString(i18n.language),
            new Date(interval.start).toLocaleTimeString(i18n.language),
            interval.end ? new Date(interval.end).toLocaleTimeString(i18n.language) : t('timeTracker.inProgress'),
    ...
    const today = new Date().toLocaleDateString(i18n.language).replace(/\//g, '-');
    const fileName = exportAll 
      ? `${t('appTitle').toLowerCase().replace(/\s/g, '-')}-${t('timeTracker.history').toLowerCase()}-${today}.csv`
      : `${t('appTitle').toLowerCase().replace(/\s/g, '-')}-${today}.csv`;
  ...
  }, [intervals, currentTime, allIntervals, i18n.language, t]);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6 transition-all duration-300 ease-in-out">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <div className="hidden md:block text-sm text-gray-500 dark:text-gray-400">
            {t('timeTracker.pressSpace')} <kbd className="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded">{t('timeTracker.spaceKey')}</kbd> {t('timeTracker.toToggle')} {isTracking ? t('timeTracker.stop') : t('timeTracker.start')}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mb-6">
          ...
          >
            {isTracking ? t('timeTracker.endTime') : t('timeTracker.startTime')}
          </button>
        </div>
